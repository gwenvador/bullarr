...
class DiscoverSearchResultsControlsTests(unittest.TestCase):
    def test_sanitized_source_results_rebind_sort_and_filter_controls(self):
        ...
        self.assertIn('function bindSearchResultsTableActions', table)
        self.assertIn('data-result-action="copyLink"', table)
        self.assertIn('data-result-action="toggleSelection"', table)
