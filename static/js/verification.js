    ...
    list.innerHTML = `
        <table class="series-table series-table-compact duplicate-series-table">
            ...
            <tbody>
                ${items.map(item => `
                    <tr class="series-table-row">
                        <td><div class="duplicate-series-entry"><strong>${duplicateSeriesNumber(item)} :</strong> ${item.cleanup_eligible ? `<input type="checkbox" class="verif-duplicate-select" value="${item.duplicate_series_id}" aria-label="Sélectionner ${escapeHtml(item.duplicate_series_title)}" onchange="verifUpdateDuplicateSelectionCount()"> ` : ''}<a href="/series/${item.duplicate_series_id}" class="missing-series-link">${escapeHtml(item.duplicate_series_title)}</a><span class="help-text duplicate-series-meta">${item.cleanup_eligible ? `Nettoyable — n°${duplicateSeriesNumber(item)} à supprimer` : 'À vérifier manuellement'}</span><span class="help-text duplicate-series-path">${escapeHtml(item.duplicate_series_path || '')}</span></div></td>
