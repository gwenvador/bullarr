...
function _renderEbdzFilesTable(missingVolumes) {
    ...
    const rows=files.map(f=>{ const name=decodeFilename(f.filename||''), key=_ebdzFileKey(f), vol=f.volume ?? f.parsed_volume, miss=vol!=null&&missing.has(vol); return `<tr class="${miss?'ebdz-modal-missing-row':''}"><td><input type="checkbox" ${_ebdzFileSelection.has(key)?'checked':''} onchange="_toggleEbdzFileSelection(${escapeForAttribute(JSON.stringify(key))},this.checked)" aria-label="Sélectionner ${escapeHtml(name)}"></td><td>${escapeHtml(buildEbdzVolumeLabel(f))}${miss?' ❌':''}</td><td>${escapeHtml(name)}</td><td>${f.resolution?escapeHtml(f.resolution):'-'}</td><td>${f.year||'-'}</td><td>${f.format?escapeHtml(f.format.toUpperCase()):'-'}</td><td>${formatBytes(parseInt(f.filesize,10)||0)}</td><td><button class="btn-icon-only search-result-download-action" onclick="addToEmule('${escapeForAttribute(f.link)}',this,'${escapeForAttribute(name)}')" data-tooltip="Ajouter à eMule/aMule" aria-label="Ajouter à eMule/aMule">${svgIcon('plus')}</button></td></tr>`; }).join('');
    el.innerHTML=`${missing.size?'<p class="ebdz-modal-missing-hint">🟡 Surligné = tome absent de votre collection</p>':''}<div style="display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap;"><button class="btn-neutral-sm" onclick="downloadSelectedEbdzFiles()" ${some?'':'disabled'}>${svgIcon('download')} Télécharger la sélection</button><span>${_ebdzFileSelection.size} sélectionné(s) · ${files.length}/${_ebdzFiles.length} affiché(s)</span></div><div style="overflow-x:auto"><table class="replace-results-table" style="width:100%;border-collapse:collapse;font-size:14px"><thead><tr class="ebdz-modal-header-row"><th><input type="checkbox" ${all?'checked':''} onchange="_toggleAllEbdzFileSelection(this.checked)" aria-label="Tout sélectionner"></th><th>Vol.<br><input type="search" value="${escapeHtml(_ebdzFileFilters.volume)}" class="search-results-filter-input th-filterable-control" placeholder="Filtrer..." oninput="_setEbdzFileFilter('volume',this.value)" aria-label="Filtrer par volume"></th><th>Fichier<br><input type="search" value="${escapeHtml(_ebdzFileFilters.filename)}" class="search-results-filter-input th-filterable-control" placeholder="Filtrer..." oninput="_setEbdzFileFilter('filename',this.value)" aria-label="Filtrer par fichier"></th><th>Résolution<br><input type="search" value="${escapeHtml(_ebdzFileFilters.resolution)}" class="search-results-filter-input th-filterable-control" placeholder="Filtrer..." oninput="_setEbdzFileFilter('resolution',this.value)" aria-label="Filtrer par résolution"></th><th>Année<br><input type="search" value="${escapeHtml(_ebdzFileFilters.year)}" class="search-results-filter-input th-filterable-control" placeholder="Filtrer..." oninput="_setEbdzFileFilter('year',this.value)" aria-label="Filtrer par année"></th><th>Format<br><select class="search-results-filter-select th-filterable-control" onchange="_setEbdzFileFilter('format',this.value)" aria-label="Filtrer par format"><option value="">Tous</option>${opts}</select></th><th>Taille</th><th>Action</th></tr></thead><tbody>${rows||'<tr><td colspan="8">Aucun résultat pour ces filtres.</td></tr>'}</tbody></table></div>`;
